        ...
        <div className="farmer-hero p-6 mb-6">
          ...
          <div className="mt-4 flex gap-3 flex-wrap">
            <div className="p-2 rounded-lg" style={{ backgroundColor: 'rgba(255,255,255,0.12)' }}>
              ...
              <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--amber-300)', fontFamily: 'Roboto Mono' }}>₹19,500</div>
            </div>
            <div className="p-2 rounded-lg" style={{ backgroundColor: 'rgba(255,255,255,0.12)' }}>
            ...
            <div className="space-y-3">
              {bankDetails.map(detail => (
                <div key={detail.key} className="flex items-center justify-between p-3 rounded-xl" style={{ backgroundColor: 'var(--neutral-100)' }}>
                  ...
                  <button onClick={() => copyToClipboard(detail.value, detail.key)} className="p-2 rounded-lg hover:bg-[var(--neutral-200)] transition-all">
        ...
        <div className="farmer-panel p-5 mb-5">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input value={amount} onChange={e => setAmount(e.target.value)} type="number" className="w-full px-4 rounded-xl border border-[var(--neutral-300)] focus:outline-none focus:border-[var(--brand-primary)]" style={{ height: '48px', fontSize: '22px', fontFamily: 'Roboto Mono', fontWeight: 700, color: 'var(--neutral-900)' }} />
            </div>
            <div>
              <label className="block mb-1.5" style={{ fontSize: '13px', fontWeight: 500, color: 'var(--neutral-700)' }}>UTR / Reference</label>
              <input value={utr} onChange={e => setUtr(e.target.value)} disabled={method === 'subsidiary'} placeholder={method === 'subsidiary' ? 'Generated by subsidiary posting' : 'Enter UTR number'} className="w-full px-4 rounded-xl border border-[var(--neutral-300)] disabled:bg-[var(--neutral-100)]" style={{ height: '48px', fontSize: '14px', fontFamily: 'Roboto Mono', color: 'var(--neutral-900)' }} />
            </div>
          </div>
          ...
          <div className="p-3 rounded-xl flex items-start gap-2 mb-4" style={{ backgroundColor: 'var(--info-100)', border: '1px solid var(--info-200)' }}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label as string} className="p-3 rounded-xl" style={{ backgroundColor: 'var(--neutral-100)', border: '1px solid var(--neutral-200)' }}>
          ...
          {method === 'direct' && (
            <button type="button" onClick={() => setReceiptUploaded(true)} className="w-full mt-4 border-2 border-dashed rounded-xl p-4 flex items-center gap-2 text-left hover:bg-[var(--success-50)]" style={{ borderColor: receiptUploaded ? 'var(--success-500)' : 'var(--neutral-300)', backgroundColor: receiptUploaded ? 'var(--success-50)' : 'var(--neutral-60)' }}>
        ...
        <aside className="col-span-4 farmer-panel p-5 sticky top-4">
          ...
          <div className="mt-3 space-y-3">
            {[
              ...
              <div key={label} className="flex items-center justify-between gap-3 py-2 border-b border-[var(--gray-200b)] last:border-b-0">
                <span style={{ fontSize: '12px', color: 'var(--neutral-550)', fontWeight: 700 }}>{label}</span>
                <span style={{ fontSize: '13px', color: 'var(--neutral-950)', fontWeight: 700, textAlign: 'right', fontFamily: label === 'Amount' || label === 'Principal' || label === 'Interest' ? 'Roboto Mono' : 'inherit' }}>{value}</span>
              </div>
            ))}
          </div>
          <div className="mt-5 p-4 rounded-xl" style={{ backgroundColor: canSubmit ? 'var(--success-50)' : 'var(--orange-50)', border: `1px solid ${canSubmit ? 'var(--success-200)' : 'var(--orange-200)'}` }}>
            <div style={{ fontSize: '13px', fontWeight: 700, color: canSubmit ? 'var(--success-700)' : 'var(--orange-900)' }}>
