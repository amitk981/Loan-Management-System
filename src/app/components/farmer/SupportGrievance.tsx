...
export function SupportGrievance({ onNavigate, activePage }: SupportGrievanceProps) {
  ...
  return (
    ...
    >
      {submitted && (
        <div className="mb-5 p-4 rounded-2xl" style={{ backgroundColor: 'var(--success-50)', border: '1px solid var(--success-200)' }}>
      ...
      {supportAction && (
        <div className="mb-5 p-4 rounded-2xl flex items-center justify-between gap-3" style={{ backgroundColor: 'var(--info-25)', border: '1px solid var(--info-200)' }}>
      ...
      <div className="grid grid-cols-5 gap-5">
        <div className="col-span-2 bg-white rounded-2xl p-5 border border-[var(--neutral-200)]">
          ...
          <div className="space-y-4">
            <div>
              ...
              <select value={category} onChange={e => setCategory(e.target.value)} className="w-full px-4 rounded-xl border border-[var(--neutral-300)] bg-white focus:outline-none" style={{ height: '44px', fontSize: '14px', color: 'var(--neutral-900)' }}>
                {categories.map(item => <option key={item}>{item}</option>)}
              </select>
            </div>
            <div>
              <label className="block mb-1.5" style={{ fontSize: '13px', fontWeight: 500, color: 'var(--neutral-700)' }}>Linked Loan</label>
              <input value="LO00000047" readOnly className="w-full px-4 rounded-xl border border-[var(--neutral-200)] bg-[var(--neutral-100)]" style={{ height: '44px', fontSize: '14px', color: 'var(--neutral-400)', fontFamily: 'Roboto Mono' }} />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea value={description} onChange={e => setDescription(e.target.value)} rows={5} className="w-full px-4 py-3 rounded-xl border border-[var(--neutral-300)] focus:outline-none resize-none" style={{ fontSize: '14px', color: 'var(--neutral-900)' }} placeholder="Describe the issue in your own words..." />
            </div>
            <button type="button" onClick={() => setAttachmentAdded(true)} className="w-full border-2 border-dashed rounded-xl p-4 flex items-center gap-2 text-left hover:bg-[var(--success-50)]" style={{ borderColor: attachmentAdded ? 'var(--success-500)' : 'var(--neutral-300)', backgroundColor: attachmentAdded ? 'var(--success-50)' : 'var(--neutral-60)' }}>
        ...
        <div id="grievance-tickets" className="col-span-3 bg-white rounded-2xl border border-[var(--neutral-200)] overflow-hidden table-scroll">
          ...
          <table className="w-full">
            ...
            <tbody>
              {grievanceRows.map(row => (
                <tr key={row.id} onClick={() => setSupportAction(`${row.id}: ${row.status} · ${row.tat}`)} className="border-b border-[var(--neutral-200)] clickable-row">
      ...
      <div className="mt-5 bg-white rounded-2xl border border-[var(--neutral-200)] overflow-hidden">
        <div className="px-5 py-3 border-b border-[var(--neutral-200)]" style={{ backgroundColor: 'var(--neutral-100)' }}>
        ...
        {[
          ...
          <div key={item.q} className="border-b border-[var(--neutral-200)] last:border-b-0">
