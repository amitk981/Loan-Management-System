...
function DocCard({ doc }: { doc: typeof farmerDocuments[number] }) {
...
export function LoanStatus({ onNavigate, activePage }: LoanStatusProps) {
        ...
        <div className="relative min-h-[560px] flex items-center justify-center overflow-hidden">
          <div className="absolute inset-0 pointer-events-none">
            {Array.from({ length: 18 }).map((_, i) => (
              <span
                key={i}
                className="absolute w-2 h-5 rounded-full animate-pulse"
                style={{
                  left: `${8 + (i * 5) % 86}%`,
                  top: `${10 + (i * 7) % 44}%`,
                  backgroundColor: i % 2 ? '#FCD34D' : '#22C55E',
          ...
          <div className="w-full max-w-xl text-center bg-white rounded-2xl p-8 border border-[#EDEEF0] shadow-sm">
            ...
            <p style={{ fontSize: '16px', color: '#9EA8B3', marginTop: '8px' }}>Congratulations, Ramesh. You have successfully repaid {farmerLoan.id}.</p>
            ...
            <div className="mt-5 text-left">
              ...
              {['SH-4 Form (returned by CS team)', 'Blank-dated Cheque (returned by CS team)'].map(item => (
                <div key={item} style={{ fontSize: '14px', color: '#166534', marginTop: '6px' }}>✓ {item}</div>
              ))}
            </div>
            <div className="mt-7 flex items-center justify-center gap-3">
              <button className="px-5 py-2.5 rounded-xl font-semibold flex items-center gap-2" style={{ backgroundColor: '#1A3C2A', color: 'white', fontSize: '14px' }}>
  ...
  if (activePage === 'farmer-loan-history') {
    ...
    return (
      <Shell activePage={activePage} onNavigate={onNavigate} breadcrumbs={['My Loans', 'Loan History']} pageTitle="Loan History" pageSubtitle="All your past and active loans with Sahyadri Farms">
        <div className="grid grid-cols-3 gap-5 mb-5">
          {[
            ['Total Borrowed', formatCurrency(totalBorrowed), '#1A3C2A'],
            ['Total Repaid', formatCurrency(totalRepaid), '#22C55E'],
            ['Active Loans', '1', '#1E88E5'],
            ...
            <tbody>
              {farmerLoanHistory.map(row => (
                <tr key={row.id} className="border-b border-[#EDEEF0]">
                  ...
                  <td className="px-5"><button onClick={() => onNavigate('farmer-active-loans')} style={{ fontSize: '13px', color: '#1E88E5', fontWeight: 700 }}>View</button></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  if (activePage === 'farmer-documents') {
    return (
      <Shell activePage={activePage} onNavigate={onNavigate} breadcrumbs={['My Documents']} pageTitle="My Documents" pageSubtitle="All documents related to your loans are stored here">
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          {['All Loans', farmerLoan.id, farmerLoan.previousLoanId].map((item, i) => (
            <button key={item} className="px-3 py-1.5 rounded-full" style={{ backgroundColor: i === 0 ? '#1A3C2A' : '#F7F8FA', color: i === 0 ? 'white' : '#3D4450', fontSize: '13px', fontWeight: 700 }}>{item}</button>
          ))}
          <input placeholder="Search documents..." className="ml-auto px-3 rounded-xl border border-[#D1D5DB]" style={{ height: '36px', fontSize: '13px', minWidth: '240px' }} />
        </div>
        <div className="grid grid-cols-3 gap-4">
          {farmerDocuments.map(doc => <DocCard key={doc.name} doc={doc} />)}
          <button className="rounded-2xl p-5 border-2 border-dashed border-[#D1D5DB] hover:bg-[#E8F5E9] text-center">
  ...
  return (
    ...
    >
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <button key={title as string} onClick={() => title === 'Make Payment' && onNavigate('farmer-repayment')} className="bg-white rounded-2xl p-5 border border-[#EDEEF0] text-left hover:bg-[#F7F8FA]">
      ...
      {activeTab === 'documents' && <div className="grid grid-cols-3 gap-4">{farmerDocuments.slice(0, 8).map(doc => <DocCard key={doc.name} doc={doc} />)}</div>}
          ...
          <div className="bg-white rounded-2xl border border-[#EDEEF0] overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody>{farmerRepaymentSchedule.map(row => (
                <tr key={row.dueDate} className="border-b border-[#EDEEF0]" style={{ backgroundColor: row.status === 'Due Soon' ? '#E8F5E9' : 'white', borderLeft: row.status === 'Due Soon' ? '3px solid #1A3C2A' : '3px solid transparent' }}>
      ...
      {activeTab === 'timeline' && (
        <div className="bg-white rounded-2xl p-6 border border-[#EDEEF0]">
          <div className="mb-5">
            ...
            <div style={{ fontSize: '13px', color: '#9EA8B3', marginTop: '3px' }}>Complete audit trail of your loan</div>
          </div>
          {farmerTimeline.map((item, i) => (
            <div key={`${item.date}-${item.action}`} className="flex gap-4 pb-5">
              ...
              <div className="flex-1 rounded-xl border border-[#EDEEF0] p-4">
                ...
                {item.doc && <button className="mt-3 flex items-center gap-1.5" style={{ fontSize: '12px', color: '#1E88E5', fontWeight: 700 }}><Download size={12} /> {item.doc}</button>}
              </div>
            </div>
          ))}
        </div>
