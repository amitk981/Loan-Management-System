...
// Embeddable 5-step loan application wizard. Shared by the farmer self-service screen
// and the Credit Manager's "Manual Loan Application Entry" (which prefixes a member search).
// `mode` flips the small set of behaviours that differ between the two surfaces; everything
// else — layout, validation, calculator, document/declaration gates — is identical so both
// roles fill the SAME form and feed the SAME pipeline (integration rule: one intake shape).
...
interface LoanApplicationFormProps {
  ...
  activePage: string;
  /** 'farmer' = self-service; 'credit' = Credit Manager filing on behalf of a member. */
  mode?: 'farmer' | 'credit';
  /** Prefill from the searched member (credit mode). Falls back to the demo farmer. */
  applicant?: Partial<Applicant>;
  breadcrumbs: string[];
  pageTitle: string;
  pageSubtitle: string;
  /** Optional content rendered above the summary strip (e.g. access note / selected member). */
  topSlot?: ReactNode;
  /** Back action when on step 1 (farmer → dashboard; credit → member search). */
...
export function LoanApplicationForm({ onNavigate, activePage, mode = 'farmer', applicant, breadcrumbs, pageTitle, pageSubtitle, topSlot, onFirstStepBack, firstStepBackLabel }: LoanApplicationFormProps) {
  const who: Applicant = { ...defaultApplicant, ...applicant };
  const isCredit = mode === 'credit';

  const [step, setStep] = useState(1);
  const [submitted, setSubmitted] = useState(false);
  const [submittedRef, setSubmittedRef] = useState('');
  // Identity is read-only for the self-serving farmer, editable for the Credit Manager.
  const [nameField, setNameField] = useState(who.fullName);
  const [mobileField, setMobileField] = useState(who.mobile);
  ...
  const [landParcels, setLandParcels] = useState<LandParcel[]>([
    { survey: '123/1', village: who.village, area: String(who.landAcres), crop: 'Grapes', season: '2025-26' },
  ]);
  ...
  const [reviewDocs, setReviewDocs] = useState([
    ...
    { name: 'NACH/ECS Mandate', uploaded: false },
    { name: 'Passport Photo', uploaded: true },
    { name: 'Nominee KYC', uploaded: false },
  ]);

  const shareValuation = who.loanValuePerShare;
  const scaleOfFinance = who.scaleOfFinance;
  ...
  const identityValid = !isCredit || (nameField.trim() !== '' && mobileField.trim() !== '');
  const step1Valid = nomineeAdult && dob.trim() !== '' && address.trim() !== '' && identityValid;
  const step3Valid = accountValid && ifscValid;
  const stepValid = (s: number) => (s === 1 ? step1Valid : s === 3 ? step3Valid : true);
  const canSubmitApplication = amountWithinLimit && allDeclarationsChecked && missingDocs.length === 0 && identityValid;
  ...
  const stepStatus = (id: number, isActive: boolean): string => {
    ...
    if (id === 1) return isCredit ? 'Identity, DOB & nominee' : 'DOB, address & nominee';
    return isActive ? 'Current section' : 'Not started';
  };
  const isUploaded = (name: string) => reviewDocs.find(d => d.name === name)?.uploaded ?? false;

  const addLandParcel = () => setLandParcels([...landParcels, { survey: '', village: '', area: '', crop: '', season: '' }]);
  const markDocUploaded = (name: string) => setReviewDocs(docs => docs.map(doc => doc.name === name ? { ...doc, uploaded: true } : doc));
  ...
  const handleSubmit = () => {
    ...
    setSubmittedRef(ref);
    setSubmitted(true);
    toast.success(isCredit ? 'Application created' : 'Application submitted', {
      description: isCredit
        ? `Reference ${ref} entered into the pipeline on behalf of ${nameField}.`
        : `Reference ${ref} created. The Credit Team will respond within the 2-day TAT.`,
    });
  };

  if (submitted) {
    return (
      <Shell activePage={activePage} onNavigate={onNavigate} breadcrumbs={[...breadcrumbs.slice(0, -1), 'Application Submitted']}>
        <div className="flex flex-col items-center justify-center min-h-96 text-center">
          <div className="w-24 h-24 rounded-full flex items-center justify-center mb-6" style={{ backgroundColor: 'var(--success-100)' }}>
            <Check size={48} style={{ color: 'var(--success-500)' }} />
          </div>
          <h2 style={{ fontSize: '28px', fontWeight: 700, color: 'var(--neutral-900)', marginBottom: '8px' }}>{isCredit ? 'Application Created' : 'Application Submitted!'}</h2>
          <p style={{ fontSize: '15px', color: 'var(--neutral-400)', marginBottom: '16px' }}>
            {isCredit
              ? `The offline application for ${nameField} now enters the same digital pipeline as farmer-submitted applications.`
              : "Your application has been received by our Credit Team. You'll hear back within 2 working days."}
          </p>
          <div className="px-6 py-4 rounded-2xl mb-6" style={{ backgroundColor: 'var(--brand-light)', border: '1px solid var(--success-100)' }}>
          ...
          <p style={{ fontSize: '14px', color: 'var(--neutral-700)', marginBottom: '24px' }}>
            {isCredit
              ? 'If documents are missing, the Credit Team follows up with the member through app notification, SMS, or phone before assessment continues.'
          ...
          <div className="flex gap-3">
            <button
              ...
              onClick={() => (isCredit ? onFirstStepBack() : onNavigate('farmer-dashboard'))}
            >
              {isCredit ? 'Create Another' : 'Back to Dashboard'}
            </button>
            <button
              ...
              onClick={() => onNavigate(isCredit ? 'credit-queue' : 'farmer-active-loans')}
            >
              {isCredit ? 'Open Application Inbox →' : 'View Application Status →'}
            </button>
          </div>
        </div>
      </Shell>
  ...
  return (
    <Shell activePage={activePage} onNavigate={onNavigate} breadcrumbs={breadcrumbs} pageTitle={pageTitle} pageSubtitle={pageSubtitle}>
          ...
          <aside className="farmer-panel p-4 sticky top-4">
            ...
            <div className="mt-5 p-4 rounded-xl" style={{ backgroundColor: 'var(--gray-50b)', border: '1px solid var(--gray-200b)' }}>
              ...
              <div style={{ fontSize: '12px', color: 'var(--neutral-550)', lineHeight: '18px', marginTop: '5px' }}>
                {step === 1 && !step1Valid ? (isCredit ? 'Enter member name, mobile, DOB and address, and confirm nominee is 18+.' : 'Enter Date of Birth and Address, and confirm nominee is 18+.') : step === 3 && !step3Valid ? 'Enter a valid Account Number and IFSC.' : step === 5 && !canSubmitApplication ? 'Finish documents, declarations, and amount checks.' : 'No blocker on this step.'}
              </div>
            </div>
          </aside>
          ...
          <main>
            ...
            {step === 1 && (
              <div className="farmer-panel p-7">
                <h2 style={{ fontSize: '20px', fontWeight: 500, color: 'var(--neutral-900)', marginBottom: '4px' }}>{isCredit ? 'Member & basic details' : 'Tell us about yourself'}</h2>
                <p style={{ fontSize: '14px', color: 'var(--neutral-400)', marginBottom: '24px' }}>Step 1 of 5 — Basic personal details</p>

                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="block mb-1.5" style={{ fontSize: '13px', fontWeight: 500, color: 'var(--neutral-700)' }}>Full Name {isCredit && <span style={{ color: 'var(--error-500)' }}>*</span>}</label>
                    <div className="relative">
                      <input value={isCredit ? nameField : who.fullName} onChange={e => setNameField(e.target.value)} readOnly={!isCredit} className="w-full px-4 rounded-xl border pr-10" style={{ height: '44px', fontSize: '14px', borderColor: 'var(--neutral-300)', backgroundColor: isCredit ? 'white' : 'var(--neutral-100)', color: isCredit ? 'var(--neutral-900)' : 'var(--neutral-400)' }} />
                      <ShieldCheck className="absolute right-3 top-3" size={16} style={{ color: 'var(--neutral-400)' }} />
                    </div>
                  </div>
                  <div>
                    <label className="block mb-1.5" style={{ fontSize: '13px', fontWeight: 500, color: 'var(--neutral-700)' }}>Mobile Number {isCredit && <span style={{ color: 'var(--error-500)' }}>*</span>}</label>
                    <input value={isCredit ? mobileField : who.mobile} onChange={e => setMobileField(e.target.value)} readOnly={!isCredit} className="w-full px-4 rounded-xl border" style={{ height: '44px', fontSize: '14px', fontFamily: 'Roboto Mono', borderColor: 'var(--neutral-300)', backgroundColor: isCredit ? 'white' : 'var(--neutral-100)', color: isCredit ? 'var(--neutral-900)' : 'var(--neutral-400)' }} />
                  </div>
                  <div>
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea rows={3} value={address} onChange={e => setAddress(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[var(--neutral-300)] focus:outline-none focus:border-[var(--brand-primary)] resize-none" style={{ fontSize: '14px', color: 'var(--neutral-900)' }} placeholder="Enter full address..." />
                  </div>
                  <div>
                    ...
                    <input type="text" defaultValue={`${who.village} / ${who.taluka}`} className="w-full px-4 rounded-xl border border-[var(--neutral-300)] focus:outline-none focus:border-[var(--brand-primary)]" style={{ height: '44px', fontSize: '14px', color: 'var(--neutral-900)' }} />
                  </div>
                  <div>
                    ...
                    <select className="w-full px-4 rounded-xl border border-[var(--neutral-300)] focus:outline-none focus:border-[var(--brand-primary)] bg-white" style={{ height: '44px', fontSize: '14px', color: 'var(--neutral-900)' }}>
                      <option>{who.district}, {who.state}</option>
                  ...
                  <div className="p-4 rounded-xl flex gap-3" style={{ backgroundColor: rekycRequested ? 'var(--warning-100)' : 'var(--success-50)', border: `1px solid ${rekycRequested ? 'var(--warning-200)' : 'var(--success-200)'}` }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '13px', color: rekycRequested ? 'var(--warning-700)' : 'var(--success-700)', lineHeight: '20px' }}>
                        {rekycRequested ? 'Compliance will refresh KYC before the Credit Team starts assessment.' : 'PAN, Aadhaar, and member records are available. A KYC refresh can still be requested if details have changed.'}
                      </div>
            ...
            {step === 2 && (
              <div className="farmer-panel p-7">
                ...
                <p style={{ fontSize: '14px', color: 'var(--neutral-400)', marginBottom: '24px' }}>Step 2 of 5 — Shareholding details and eligibility</p>

                <div className="grid grid-cols-3 gap-4 mb-6">
                  {[
                    { label: 'Folio Number', value: who.folioNo, mono: true },
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div className="bg-white rounded-xl p-4">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <input type="number" value={shares} onChange={e => setShares(parseInt(e.target.value) || 0)} className="w-20 px-2 rounded-lg border border-[var(--neutral-300)] text-center focus:outline-none focus:border-[var(--brand-primary)]" style={{ height: '36px', fontSize: '14px', fontFamily: 'Roboto Mono' }} />
                        <span style={{ fontSize: '12px', color: 'var(--neutral-400)' }}>shares × ₹{shareValuation}</span>
                      </div>
                      <div style={{ fontSize: '22px', fontWeight: 700, color: 'var(--success-500)', fontFamily: 'Roboto Mono' }}>{formatCurrency(method1)}</div>
                    ...
                    <div className="bg-white rounded-xl p-4">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <input type="number" value={landAcres} step="0.5" onChange={e => setLandAcres(parseFloat(e.target.value) || 0)} className="w-20 px-2 rounded-lg border border-[var(--neutral-300)] text-center focus:outline-none focus:border-[var(--brand-primary)]" style={{ height: '36px', fontSize: '14px', fontFamily: 'Roboto Mono' }} />
                        <span style={{ fontSize: '12px', color: 'var(--neutral-400)' }}>acres × ₹{scaleOfFinance.toLocaleString('en-IN')}/acre</span>
                      </div>
                      <div style={{ fontSize: '22px', fontWeight: 700, color: 'var(--success-500)', fontFamily: 'Roboto Mono' }}>{formatCurrency(method2)}</div>
                      <div style={{ fontSize: '11px', color: 'var(--neutral-400)', marginTop: '4px' }}>Scale of Finance — FY 2025-26</div>
                    </div>
                  </div>
                  <div className="mt-4 p-4 rounded-xl flex items-center justify-between" style={{ backgroundColor: 'var(--brand-primary)' }}>
                    <span style={{ fontSize: '14px', color: 'rgba(255,255,255,0.8)' }}>Maximum Eligible Loan Amount</span>
                    <span style={{ fontSize: '24px', fontWeight: 700, color: 'white', fontFamily: 'Roboto Mono' }}>{formatCurrency(eligibleLimit)}</span>
                  </div>
                  <p style={{ fontSize: '12px', color: 'var(--brand-secondary)', marginTop: '8px' }}>
                    Lower of Method 1 ({formatCurrency(method1)}) and Method 2 ({formatCurrency(method2)}). Final amount subject to Credit Assessment review.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="number" value={requestedAmount} onChange={e => setRequestedAmount(e.target.value)} placeholder={`Max: ${formatCurrency(eligibleLimit)}`} className="w-full px-4 rounded-xl border border-[var(--neutral-300)] focus:outline-none focus:border-[var(--brand-primary)]" style={{ height: '44px', fontSize: '14px', color: 'var(--neutral-900)', fontFamily: 'Roboto Mono' }} />
                    {reqAmount > eligibleLimit && <p style={{ fontSize: '12px', color: 'var(--error-500)', marginTop: '4px' }}>Amount exceeds eligible limit of {formatCurrency(eligibleLimit)}</p>}
                  </div>
                  <div>
              ...
              <div className="farmer-panel p-7">
                ...
                {[
                  ...
                  <div key={i} className="mb-4">
                    ...
                    <button type="button" onClick={() => markDocUploaded(zone.doc)} className="w-full border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer transition-all hover:border-[var(--brand-secondary)] hover:bg-[var(--success-50)]" style={{ borderColor: isUploaded(zone.doc) ? 'var(--success-500)' : 'var(--neutral-300)', padding: '20px', backgroundColor: isUploaded(zone.doc) ? 'var(--success-50)' : 'var(--neutral-60)' }}>
                          ...
                          <tr key={i} className="border-b border-[var(--neutral-200)]">
                            {(['survey', 'village', 'area', 'crop', 'season'] as const).map(field => (
                              <td key={field} className="px-2 py-2">
                                <input value={parcel[field]} onChange={e => { const updated = [...landParcels]; updated[i] = { ...updated[i], [field]: e.target.value }; setLandParcels(updated); }} className="w-full px-2 rounded-lg border border-[var(--neutral-200)] focus:outline-none focus:border-[var(--brand-primary)]" style={{ height: '36px', fontSize: '13px', color: 'var(--neutral-900)' }} />
                              </td>
                            ))}
                            <td className="px-2">
                ...
                <div className="mt-5 pt-5 border-t border-[var(--neutral-200)]">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="text" value={ifsc} onChange={e => setIfsc(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 11))} placeholder="ABCD0123456" className="w-full px-4 rounded-xl border focus:outline-none focus:border-[var(--brand-primary)]" style={{ height: '44px', fontSize: '14px', color: 'var(--neutral-900)', fontFamily: 'Roboto Mono', borderColor: ifsc && !ifscValid ? 'var(--error-500)' : 'var(--neutral-300)' }} />
                      {ifsc !== '' && !ifscValid && <p style={{ fontSize: '12px', color: 'var(--error-500)', marginTop: '4px' }}>IFSC format: 4 letters, 0, then 6 characters.</p>}
                    </div>
                    <div>
              ...
              <div className="farmer-panel p-7">
                ...
                <div className="pt-5 border-t border-[var(--neutral-200)]">
                  ...
                  <div className="space-y-3">
                    {[
                      { key: 'd1' as const, text: isCredit ? 'The member declares they are not a wilful defaulter and have no pending dues with any financial institution.' : 'I declare that I am not a wilful defaulter and have no pending dues with any financial institution.' },
                      { key: 'd2' as const, text: isCredit ? 'The member consents to CKYC and credit bureau enquiries as required for loan processing.' : 'I consent to CKYC and credit bureau enquiries as required for loan processing.' },
                      { key: 'd3' as const, text: isCredit ? 'The pledged asset (shares) is not already encumbered or pledged elsewhere.' : 'I declare that the pledged asset (shares) is not already encumbered or pledged elsewhere.' },
                    ].map(decl => (
                      <label key={decl.key} className="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" checked={declarations[decl.key]} onChange={e => setDeclarations({ ...declarations, [decl.key]: e.target.checked })} className="mt-0.5 w-4 h-4 rounded" style={{ accentColor: 'var(--brand-primary)' }} />
            ...
            {step === 5 && (
              <div className="farmer-panel p-7">
                ...
                {[
                  { title: 'Personal Details', content: `Name: ${isCredit ? nameField : who.fullName} · Mobile: ${isCredit ? mobileField : who.mobile} · Village: ${who.village}, ${who.district}`, editStep: 1 },
                  { title: 'Shareholding Details', content: `Folio: ${who.folioNo} · ${shares} shares · Share Type: Physical`, editStep: 2 },
                ...
                <div className="mt-5 pt-5 border-t border-[var(--neutral-200)]">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {reviewDocs.map((doc, i) => (
                      <button key={i} type="button" onClick={() => { const updated = [...reviewDocs]; updated[i] = { ...updated[i], uploaded: !updated[i].uploaded }; setReviewDocs(updated); }} className="flex items-center gap-2 p-2 rounded-lg text-left" style={{ backgroundColor: doc.uploaded ? 'var(--success-50)' : 'var(--error-50)' }}>
                ...
                <div className="mt-5">
                  <GateBanner
                    ...
                    detail={canSubmitApplication
                      ...
                      : [
                          !identityValid && 'Enter the member name and mobile number.',
                  ...
                  <p className="mt-3 px-1" style={{ fontSize: '13px', color: 'var(--neutral-700)', lineHeight: '20px' }}>
                    {isCredit
                      ? 'By creating this application, you confirm the member authorises SFPCL to verify details, conduct credit bureau enquiries, and process this application per Section 378ZK of the Companies Act, 2013.'
                ...
                <button onClick={handleSubmit} disabled={!canSubmitApplication} className="w-full mt-5 py-3 rounded-xl font-medium transition-all hover:opacity-90 flex items-center justify-center gap-2" style={{ backgroundColor: canSubmitApplication ? 'var(--brand-primary)' : 'var(--neutral-400)', color: 'white', fontSize: '15px', cursor: canSubmitApplication ? 'pointer' : 'not-allowed' }}>
                  {isCredit ? 'Create Application →' : 'Submit Application →'}
                </button>
              </div>
            )}
            ...
            <div className="flex items-center justify-between mt-5">
              <button onClick={() => (step > 1 ? setStep(step - 1) : onFirstStepBack())} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-[var(--neutral-200)] transition-all hover:bg-[var(--neutral-100)]" style={{ fontSize: '14px', color: 'var(--neutral-700)' }}>
                <ChevronLeft size={16} /> {step > 1 ? 'Previous' : firstStepBackLabel}
              </button>
              {step < 5 && (
                <button onClick={() => stepValid(step) && setStep(step + 1)} disabled={!stepValid(step)} title={!stepValid(step) ? 'Complete the required fields marked * on this step to continue.' : undefined} className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium transition-all hover:opacity-90" style={{ backgroundColor: !stepValid(step) ? 'var(--neutral-400)' : 'var(--brand-primary)', color: 'white', fontSize: '14px', cursor: !stepValid(step) ? 'not-allowed' : 'pointer' }}>
