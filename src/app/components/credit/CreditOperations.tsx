    ...
    return (
      <div className="space-y-5">
          ...
          <SimpleTable headers={['Loan ID', 'Name', 'Amount', 'Submitted', 'SC Level', 'Status']}>
            {sanctionQueue.map(row => (
              <tr key={row.id} onClick={() => setSelectedSc(row.id)} className="border-b border-[#E5E7EB] hover:bg-[#FAFAF8] cursor-pointer" style={{ backgroundColor: selectedSc === row.id ? '#E8F1FA' : 'white' }}>
                ...
                <Cell><span className="px-2 py-1 rounded-full" style={{ backgroundColor: row.level === 'CFO+2Dir' ? '#F3E8FF' : '#E8F1FA', color: row.level === 'CFO+2Dir' ? 'var(--accent-sanction)' : 'var(--brand-accent)', fontSize: '11px', fontWeight: 800 }}>{row.level}</span></Cell>
        ...
        <div className="bg-white rounded-lg p-5 border border-[#E5E7EB]">
          ...
          {selected.status === 'Rejected' && <button className="mt-4 px-4 py-2.5 rounded-lg font-semibold" style={{ backgroundColor: 'var(--error-700)', color: 'white', fontSize: '14px' }}>Prepare Rejection Note →</button>}
        </div>
      </div>
    );
  };
      ...
      <SimpleTable headers={['#', 'Loan ID', 'Borrower', 'Amount', 'Disbursed', 'Stage', 'DPD', 'Action']}>
        {loanRegister.map((row, i) => (
          <tr key={row.id} onClick={() => setSelectedLoan(row.id)} className="border-b border-[#E5E7EB] clickable-row">
            ...
            <Cell><button onClick={(e) => { e.stopPropagation(); onNavigate('loan-file'); }} className="px-3 py-1.5 rounded-md" style={{ backgroundColor: 'var(--neutral-150)', color: 'var(--brand-accent)', fontSize: '12px', fontWeight: 800 }}>Open File</button></Cell>
          </tr>
        ))}
      </SimpleTable>
      ...
      <div className="grid grid-cols-5 gap-4">
        {dpdSummary.map(item => (
          <div key={item.label} className="bg-white rounded-lg p-4 border border-[#E5E7EB]">
            <div style={{ fontSize: '12px', color: 'var(--neutral-500)', fontWeight: 800 }}>{item.label}</div>
        ...
        <SimpleTable headers={['Borrower', 'Loan ID', 'Amount', 'DPD Days', 'Action Required']}>
          {dpdRows.map(row => (
            <tr key={row.loan} onClick={() => setSelectedLoan(row.loan)} className="border-b border-[#E5E7EB] clickable-row">
              ...
              <Cell right mono>{formatCurrency(row.amount)}</Cell>
              <Cell><span style={{ color: 'var(--error-700)', fontWeight: 700 }}>{row.dpd} d</span></Cell>
...
function SimpleTable({ headers, children }: { headers: string[]; children: React.ReactNode }) {
  return <div className="table-scroll"><table className="w-full"><thead><tr>{headers.map(h => <th key={h} className="px-4 py-3 text-left" style={{ fontSize: '11px', color: 'var(--neutral-500)', fontWeight: 700, textTransform: 'uppercase' }}>{h}</th>)}</tr></thead><tbody>{children}</tbody></table></div>;
}
