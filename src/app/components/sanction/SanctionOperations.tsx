...
export function SanctionOperations({ onNavigate, activePage }: SanctionOperationsProps) {
  ...
  const renderSpecial = () => (
    <div className="bg-white rounded-lg p-5 border-2" style={{ borderColor: 'var(--gold-500)', borderTopWidth: 4 }}>
      ...
      <div className="mt-5 p-4 rounded-lg" style={{ backgroundColor: 'var(--warning-50)', border: '1px solid var(--warning-200)', color: 'var(--warning-700)', fontSize: '13px', fontWeight: 700 }}>Current blocker: GM resolution not yet uploaded. Disbursement is locked until step 4 is complete.</div>
    </div>
  );

  const renderRegister = () => (
    <DataCard title="Credit Sanction Register" action={<div className="flex gap-2"><button className="px-3 py-1.5 rounded-lg border border-[var(--neutral-200)]">Export CSV</button><button className="px-3 py-1.5 rounded-lg border border-[var(--neutral-200)]">Export PDF</button></div>}>
      <div className="p-4 border-b border-[var(--neutral-200)] flex gap-2 flex-wrap">{['All', 'Approved', 'Rejected', 'Returned', 'CFO+1Dir', 'CFO+2Dir', 'Flagged only'].map(f => <button key={f} onClick={() => setRegisterFilter(f)} className="px-3 py-1.5 rounded-full" style={{ backgroundColor: registerFilter === f ? 'var(--accent-sanction)' : 'var(--neutral-100)', color: registerFilter === f ? 'white' : 'var(--neutral-700)', fontSize: '12px', fontWeight: 700 }}>{f}</button>)}</div>
      <SimpleTable headers={['Date', 'Loan ID', 'Borrower', 'Amount', 'Decision', 'Authority']}>
        {sanctionRegister.map(row => <tr key={row.loan} onClick={() => onNavigate('sc-register')} className="border-b border-[var(--neutral-200)] clickable-row"><Cell>{row.date}</Cell><Cell mono blue>{row.loan}</Cell><Cell>{row.borrower}</Cell><Cell mono right>{formatCurrency(row.amount)}</Cell><Cell><StatusBadge status={row.decision} /></Cell><Cell>{row.authority}</Cell></tr>)}
      </SimpleTable>
      <div className="p-4 border-t border-[var(--neutral-200)]" style={{ fontSize: '13px', color: 'var(--neutral-700)' }}><strong>Expanded row example:</strong> LO00000091 signed by S. Nair (CFO) and R. Deshmukh (Director). 7-point checklist all confirmed. Sanction Note Ref: LAN-LO91-2025. Rows are immutable.</div>
    </DataCard>
  );

  const renderApprovalQueue = () => (
    <DataCard title={activePage === 'sc-my-sign' ? 'Awaiting S. Nair Signature' : 'Sanction Committee Approval Queue'} action={<button className="px-3 py-1.5 rounded-lg border border-[var(--neutral-200)]">Sort: Oldest first</button>}>
      <div className="p-4 border-b border-[var(--neutral-200)] flex gap-2 flex-wrap">
      ...
      <SimpleTable headers={['Priority', 'Loan ID', 'Borrower', 'Amount', 'Appraisal', 'Authority', 'Waiting', 'Action']}>
        {scApprovalQueue.map(row => (
          <tr key={row.id} onClick={() => onNavigate(row.page)} className="border-b border-[var(--neutral-200)] clickable-row">
      ...
      <div className="p-4 border-t border-[var(--neutral-200)]" style={{ fontSize: '13px', color: 'var(--neutral-700)' }}>Hard rule: SC cannot submit a decision until all seven scrutiny checks are completed and the Credit Sanction Register checkbox is confirmed.</div>
    </DataCard>
  );
  ...
  const renderExceptions = () => (
    <DataCard title="Exception Register" action={<button className="px-3 py-1.5 rounded-lg" style={{ backgroundColor: 'var(--accent-sanction)', color: 'white', fontSize: '12px', fontWeight: 700 }}>+ Log Exception</button>}>
      <div className="p-4 border-b border-[var(--neutral-200)] flex gap-2 flex-wrap">{['All', 'Limit Exceeded', 'Special Approval', 'Director Case', 'Policy Override', 'Extension Granted'].map(f => <button key={f} onClick={() => setExceptionFilter(f)} className="px-3 py-1.5 rounded-full" style={{ backgroundColor: exceptionFilter === f ? 'var(--accent-sanction)' : 'var(--neutral-100)', color: exceptionFilter === f ? 'white' : 'var(--neutral-700)', fontSize: '12px', fontWeight: 700 }}>{f}</button>)}</div>
      <SimpleTable headers={['Date', 'Loan ID', 'Exception Type', 'Approver', 'Status']}>
        {scExceptions.map(row => <tr key={row.loan} onClick={() => onNavigate('sc-exceptions')} className="border-b border-[var(--neutral-200)] clickable-row"><Cell>{row.date}</Cell><Cell mono blue>{row.loan}</Cell><Cell>{row.type}</Cell><Cell>{row.approver}</Cell><Cell><StatusBadge status={row.status} /></Cell></tr>)}
      </SimpleTable>
    </DataCard>
  );
      ...
      <DataCard title="Portfolio at Risk — Days Past Due Buckets" action={<button className="px-3 py-1.5 rounded-lg" style={{ backgroundColor: 'var(--accent-sanction)', color: 'white', fontSize: '12px', fontWeight: 700 }}>Download Board Pack PDF</button>}>
        <div className="p-5 space-y-4">
          {[
            ...
            ['2-3 years overdue', '5 loans', '₹7,20,000', 3.9, 'var(--warning-800)'],
  ...
  const renderDefault = () => (
    <div className="bg-white rounded-lg p-5 border border-[var(--neutral-200)]">
      ...
      <div className="mt-5 p-4 rounded-lg" style={{ backgroundColor: 'var(--neutral-100)', border: '1px solid var(--neutral-200)' }}>
        ...
        <textarea className="w-full mt-3 p-3 rounded-lg border border-[var(--neutral-300)]" rows={3} placeholder="SC Authorisation Notes (mandatory)" />
  ...
  const renderReturns = () => (
    <div className="bg-white rounded-lg p-5 border border-[var(--neutral-200)] max-w-4xl">
      ...
      <textarea className="w-full mt-3 p-3 rounded-lg border border-[var(--neutral-300)]" rows={4} placeholder="Additional instructions to Credit Team" />
      ...
      <button className="mt-4 px-4 py-2.5 rounded-lg font-medium" style={{ backgroundColor: 'var(--navy-800)', color: 'white' }}>Return Application →</button>
    </div>
  );
...
function renderGeneric(onNavigate: (page: string) => void) {
  ...
  ].map(([title, page]) => <button key={title} onClick={() => onNavigate(page)} className="bg-white rounded-lg p-5 border border-[var(--neutral-200)] text-left clickable-card"><FileText size={18} color="var(--accent-sanction)" /><h3 style={{ fontSize: '15px', fontWeight: 700, marginTop: '12px' }}>{title}</h3><div style={{ fontSize: '13px', color: 'var(--neutral-700)', lineHeight: '20px', marginTop: '8px' }}>Resolution references and authority rules.</div><span className="inline-flex mt-4 px-3 py-1.5 rounded-lg" style={{ backgroundColor: 'var(--accent-sanction-100)', color: 'var(--accent-sanction)', fontSize: '12px', fontWeight: 700 }}><Download size={13} style={{ marginRight: 6 }} />Export</span></button>)}</div>;
}

function DataCard({ title, action, children }: { title: string; action?: ReactNode; children: ReactNode }) {
  return <div className="bg-white rounded-lg border border-[var(--neutral-200)] overflow-hidden"><div className="px-5 py-3 border-b border-[var(--neutral-200)] flex items-center justify-between" style={{ backgroundColor: 'var(--neutral-100)' }}><h3 style={{ fontSize: '15px', fontWeight: 700 }}>{title}</h3>{action}</div>{children}</div>;
}
...
function Info({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between py-2 border-b border-[var(--neutral-200)]"><span style={{ fontSize: '13px', color: 'var(--neutral-500)' }}>{label}</span><span style={{ fontSize: '13px', color: 'var(--neutral-900)', fontFamily: 'Roboto Mono', fontWeight: 700 }}>{value}</span></div>;
}
