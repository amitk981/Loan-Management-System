...
export function ApprovalScreen({ onNavigate, activePage }: ApprovalScreenProps) {
  ...
  return (
    ...
    >
      ...
      {isJoint && (
        <div className="mb-5 p-4 rounded-lg" style={{ backgroundColor: 'var(--warning-100)', borderLeft: '4px solid #D97706' }}>
      ...
      <div className="grid grid-cols-12 gap-5">
        <div className="col-span-5 space-y-4">
          ...
          <Panel title="Loan Eligibility Calculation">
            ...
            <div className="col-span-2 p-4 rounded-lg" style={{ backgroundColor: 'var(--brand-light)', border: '1px solid #BBF7D0' }}>
        ...
        <div className="col-span-4 space-y-4">
          ...
          <Panel title="Evidence">
            {['7/12 Extract uploaded', 'Crop Plan uploaded', '6 months Bank Statement uploaded', 'PoA prepared by CS', 'No Director/Relative borrower'].map(item => <div key={item} className="p-3 border-b border-[#EDEEF0] flex items-center gap-2" style={{ fontSize: '13px', color: 'var(--neutral-700)' }}><CheckCircle size={14} color="var(--success-500)" />{item}</div>)}
          </Panel>
        </div>
            ...
            <div className="p-4 border-b border-[#EDEEF0]">
              ...
              <div className="mt-2 space-y-2">
                {(isJoint ? ['S. Nair · CFO · Signed', 'R. Deshmukh · Director · Pending', 'V. Kulkarni · Director · Pending'] : ['S. Nair · CFO · You', 'R. Deshmukh · Director · Pending']).map((sig, i) => (
                  <div key={sig} className="px-2 py-1 rounded-full inline-flex mr-1" style={{ backgroundColor: i === 0 ? 'var(--success-100)' : 'var(--warning-100)', color: i === 0 ? '#166534' : '#92400E', fontSize: '11px', fontWeight: 800 }}>{sig}</div>
                ))}
              </div>
            </div>
            <div className="p-4">
              ...
              {scScrutinyItems.map(([label, note], i) => (
                <label key={label} className="block p-2 rounded-lg mb-2 cursor-pointer" style={{ backgroundColor: scrutiny[i] ? '#F0FDF4' : 'white', border: '1px solid #EDEEF0' }}>
                  <div className="flex items-start gap-2">
                    ...
                    <div><div style={{ fontSize: '12px', color: scrutiny[i] ? '#166534' : 'var(--neutral-700)', fontWeight: 800 }}>{i + 1}. {label}</div><div style={{ fontSize: '11px', color: '#6B7280', marginTop: '3px' }}>{note}</div></div>
                  </div>
                </label>
              ))}
              ...
              <label className="flex items-start gap-2 mt-4"><input type="checkbox" checked={registerChecked} onChange={e => setRegisterChecked(e.target.checked)} style={{ accentColor: '#7C3AED', marginTop: '2px' }} /><span style={{ fontSize: '12px', color: 'var(--neutral-700)' }}>Record in Credit Sanction Register (mandatory)</span></label>
              <button disabled={!canSubmit} onClick={() => setShowConfirm(true)} className="w-full mt-4 py-3 rounded-lg font-semibold" style={{ backgroundColor: canSubmit ? '#7C3AED' : 'var(--neutral-400)', color: 'white', fontSize: '14px', cursor: canSubmit ? 'pointer' : 'not-allowed' }}>Submit Decision →</button>
            </div>
          </div>
        </div>
      </div>
...
function Panel({ title, children }: { title: string; children: ReactNode }) {
  return <div className="bg-white rounded-lg border border-[#EDEEF0] overflow-hidden"><div className="px-4 py-3 border-b border-[#EDEEF0]" style={{ backgroundColor: 'var(--neutral-100)', fontSize: '14px', fontWeight: 700, color: 'var(--neutral-900)' }}>{title}</div><div className="grid grid-cols-2 gap-0">{children}</div></div>;
}
