...
export function DisbursementScreen({ onNavigate, activePage }: DisbursementScreenProps) {
  ...
  return (
    <Shell activePage={activePage} onNavigate={onNavigate} breadcrumbs={['Treasury', 'Disbursement Queue', disbursementLoan.id]} pageTitle={`Disbursement — ${disbursementLoan.id}`} pageSubtitle={`${disbursementLoan.borrower} · ${formatCurrency(disbursementLoan.amount, true)}`}>
      <div className="sticky top-0 z-10 bg-white border border-[var(--neutral-200)] rounded-lg p-4 mb-5">
      ...
      {step === 1 && (
        <div className="grid grid-cols-5 gap-5">
          <div className="col-span-3 bg-white rounded-lg border border-[var(--neutral-200)] overflow-hidden flex flex-col">
            <div className="px-5 py-4 border-b border-[var(--neutral-200)] flex items-center justify-between">
              ...
              <label className="flex items-center gap-2 cursor-pointer bg-[var(--neutral-100)] px-3 py-1.5 rounded-lg border border-[var(--neutral-200)]">
            ...
            {!gatesPassed && (
              <div className="p-4" style={{ borderBottom: '1px solid var(--error-200)' }}>
                <GateBanner
                  variant="blocked"
            ...
            <div className="p-5 space-y-3 flex-1">
              {preflightGates.map(([label, note, link], i) => {
                ...
                return (
                  <div key={label} className="flex items-center gap-3 p-3 rounded-lg" style={{ backgroundColor: isFailedGate ? 'var(--warning-50)' : 'var(--success-50)', borderLeft: `3px solid ${isFailedGate ? 'var(--warning-500)' : 'var(--success-500)'}` }}>
            ...
            <div className="p-5 border-t border-[var(--neutral-200)] bg-[var(--neutral-100)]">
      ...
      {step === 2 && (
        <div className="max-w-4xl mx-auto bg-white rounded-lg p-6 border border-[var(--neutral-200)]">
          <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 16 }}>Beneficiary Bank Account Verification</h3>
          <div className="border-2 border-[var(--neutral-200)] rounded-lg overflow-hidden">
            <div className="p-4 border-b border-[var(--neutral-200)]" style={{ backgroundColor: 'var(--neutral-100)' }}><strong>Beneficiary Bank Details — {disbursementLoan.borrower}</strong><div style={{ fontSize: 12, color: 'var(--neutral-500)' }}>Source: Application Form (Cancelled Cheque)</div></div>
            ...
            <div className="p-4 grid grid-cols-2 gap-3">
              ...
              <div className="p-3 rounded-lg" style={{ backgroundColor: 'var(--info-50)', color: 'var(--accent-treasury-700)', fontSize: 13, fontWeight: 700 }}>✓ Penny drop verification passed</div>
            </div>
          </div>
          <div className="mt-4"><label style={{ fontSize: 13, fontWeight: 700 }}>Debit From (SFPCL Account)</label><select className="w-full mt-1 px-3 rounded-lg border border-[var(--neutral-300)] bg-white" style={{ height: 42 }}><option>SFPCL — RBL Bank · A/C XXXX-XXXX-8842 · Operating A/C</option></select><p style={{ fontSize: 12, color: 'var(--neutral-500)', marginTop: 5 }}>This is the company's designated disbursement account per SOP §5.3</p></div>
          <div className="flex gap-3 mt-5"><button onClick={() => setStep(1)} className="px-4 py-2.5 rounded-lg border border-[var(--neutral-200)]">← Back</button><button onClick={() => setStep(3)} className="flex-1 py-2.5 rounded-lg font-medium" style={{ backgroundColor: 'var(--accent-treasury)', color: 'white' }}>Proceed to Payment Initiation →</button></div>
        </div>
      )}

      {step === 3 && (
        <div className="grid grid-cols-11 gap-5">
          <div className="col-span-6 bg-white rounded-lg p-5 border border-[var(--neutral-200)]">
            ...
            <div className="mb-4">
              <label style={{ fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>Payment Mode <span className="px-2 py-0.5 rounded-full" style={{ backgroundColor: 'var(--neutral-175)', color: 'var(--neutral-500)', fontSize: 10 }}>Auto-selected</span></label>
            ...
            <div className="mb-4"><label style={{ fontSize: 13, fontWeight: 700 }}>Transaction Remarks</label><input value={remarks} onChange={e => setRemarks(e.target.value.slice(0, 100))} className="w-full mt-1 px-3 rounded-lg border border-[var(--neutral-300)]" style={{ height: 42 }} /><div style={{ fontSize: 11, color: 'var(--neutral-500)', textAlign: 'right' }}>{remarks.length}/100</div></div>
            <Field label="Initiation Date & Time" value="10 Jun 2026 · 10:15 AM" />
            <div className="p-3 rounded-lg mb-4" style={{ backgroundColor: 'var(--info-50)', color: 'var(--accent-treasury-700)', fontSize: 13, fontWeight: 700 }}>Chief Financial Controller authorization required</div>
            <div className="flex gap-3"><button onClick={() => setStep(2)} className="px-4 py-2.5 rounded-lg border border-[var(--neutral-200)]">← Back</button><button onClick={() => setStep(4)} className="flex-1 py-2.5 rounded-lg font-medium" style={{ backgroundColor: 'var(--accent-treasury)', color: 'white' }}><Send size={15} style={{ display: 'inline', marginRight: 6 }} />Send to Finance Controller for Authorization →</button></div>
          </div>
          <div className="col-span-5 bg-[var(--neutral-100)] rounded-lg p-5 border border-[var(--neutral-200)] sticky top-24 self-start">
      ...
      {step === 4 && (
        <div className="max-w-3xl mx-auto bg-white rounded-lg p-6 border border-[var(--neutral-200)]">
      ...
      {step === 5 && (
        <div className="max-w-3xl mx-auto bg-white rounded-lg p-6 border border-[var(--neutral-200)]">
          ...
          <pre className="rounded-lg p-5 overflow-x-auto" style={{ backgroundColor: 'var(--terminal-900)', color: 'white', fontFamily: 'Roboto Mono', fontSize: 13, lineHeight: '22px' }}>{`SAP Journal Entry — Auto-Generated
═══════════════════════════════════════
...
Posted by: Rajesh Kulkarni`}</pre>
      ...
      {step === 6 && (
        <div className="max-w-2xl mx-auto bg-white rounded-lg p-8 border-2 border-[var(--success-500)] text-center">
          ...
          <div className="text-left p-4 rounded-lg mt-5" style={{ backgroundColor: 'var(--success-50)', border: '1px solid var(--success-200)' }}>
            {[
          ...
          <div className="flex gap-3 mt-6"><button className="flex-1 py-2.5 rounded-lg" style={{ backgroundColor: 'var(--brand-primary)', color: 'white' }}>Download Disbursement Advice</button><button onClick={() => onNavigate('treasury-pending')} className="flex-1 py-2.5 rounded-lg border border-[var(--neutral-200)]">Go to Queue</button><button onClick={() => onNavigate('treasury-dashboard')} className="flex-1 py-2.5 rounded-lg border border-[var(--neutral-200)]">Dashboard</button></div>
        </div>
      )}
    </Shell>
  );
...
function LoanSummary() {
  return (
    <div className="col-span-2 bg-white rounded-lg p-5 border border-[var(--neutral-200)] sticky top-24 self-start">
      ...
      <div className="border-t border-[var(--neutral-200)] mt-4 pt-4" style={{ fontSize: 13, color: 'var(--success-700)', lineHeight: '24px' }}>✓ SH-4 Form held<br />✓ Blank cheque in custody<br />✓ PoA notarized</div>
    </div>
  );
}

function Info({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return <div className="p-3 border-b border-r border-[var(--neutral-200)]"><div style={{ fontSize: 11, color: 'var(--neutral-400)', fontWeight: 700 }}>{label}</div><div style={{ fontSize: 13, color: 'var(--neutral-900)', fontWeight: 700, marginTop: 4, fontFamily: mono ? 'Roboto Mono' : 'inherit' }}>{value}</div></div>;
}

function Field({ label, value, note, mono, large }: { label: string; value: string; note?: string; mono?: boolean; large?: boolean }) {
  return <div className="mb-4"><label style={{ fontSize: 13, fontWeight: 700 }}>{label}</label><div className="mt-1 px-3 rounded-lg border border-[var(--neutral-200)] bg-[var(--neutral-100)] flex items-center" style={{ height: large ? 56 : 42, fontFamily: mono ? 'Roboto Mono' : 'inherit', fontSize: large ? 24 : 14, fontWeight: 700 }}>{value}</div>{note && <div style={{ fontSize: 12, color: 'var(--neutral-500)', marginTop: 4 }}>{note}</div>}</div>;
}

function Preview({ label, value }: { label: string; value: string }) {
  return <div className="flex justify-between gap-4 py-2 border-b border-[var(--neutral-200)] last:border-0"><span style={{ fontSize: 11, color: 'var(--neutral-400)', fontWeight: 700 }}>{label}</span><span style={{ fontSize: 13, color: 'var(--neutral-900)', fontWeight: 700, textAlign: 'right', fontFamily: label.includes('AMOUNT') || label.includes('Reference') || label.includes('UTR') ? 'Roboto Mono' : 'inherit' }}>{value}</span></div>;
}
