...
function Metric({ label, value, note, color }: { label: string; value: string; note: string; color: string }) {
  return <div className="bg-white rounded-lg p-4 border border-[#EDEEF0]"><div style={{ fontSize: 12, color: 'var(--neutral-400)', fontWeight: 700 }}>{label}</div><div style={{ fontSize: 25, color, fontWeight: 700, fontFamily: 'Roboto Mono', marginTop: 4 }}>{value}</div><div style={{ fontSize: 12, color: 'var(--neutral-700)', marginTop: 4 }}>{note}</div></div>;
}
...
export function TreasuryOperations({ onNavigate, activePage }: TreasuryOperationsProps) {
      ...
      return (
        <div className="grid grid-cols-12 gap-5">
          ...
          <div className="col-span-4 bg-white rounded-lg p-5 border border-[#EDEEF0]">
            ...
            <div className="grid grid-cols-3 gap-2 my-4">{['View Sanction', 'View Checklist', 'SAP Preview'].map(x => <button key={x} className="p-2 rounded-lg border border-[#EDEEF0]" style={{ fontSize: 11, fontWeight: 700 }}>{x}</button>)}</div>
      ...
      <div className="space-y-5">
        ...
        <DataCard title="Disbursement Control Queue" action={<button onClick={() => onNavigate('treasury-disbursement')} className="px-3 py-1.5 rounded-lg" style={{ backgroundColor: 'var(--accent-treasury)', color: 'white', fontSize: 12, fontWeight: 700 }}>Process Selected</button>}>
          <SimpleTable headers={['Loan ID', 'Borrower', 'Amount', 'Stage', 'CS Sign', 'SAP Code', 'Waiting', 'Action']}>
            {rows.map(row => <tr key={`${row.id}-${row.stage}`} onClick={() => onNavigate(row.page)} className="border-b border-[#EDEEF0] clickable-row"><Cell mono blue>{row.id}</Cell><Cell>{row.borrower}</Cell><Cell mono right>{formatCurrency(row.amount, true)}</Cell><Cell><StatusBadge status={row.stage} /></Cell><Cell>{row.csSign}</Cell><Cell mono>{row.sapCode || 'Pending'}</Cell><Cell>{row.waiting}</Cell><Cell><button onClick={(e) => { e.stopPropagation(); onNavigate(row.page); }} className="px-3 py-1.5 rounded-lg" style={{ backgroundColor: row.sapCode ? 'var(--accent-treasury)' : 'var(--neutral-200)', color: row.sapCode ? 'white' : 'var(--neutral-500)', fontSize: 12, fontWeight: 700 }}>{row.action}</button></Cell></tr>)}
          </SimpleTable>
        </DataCard>
      </div>
    );
    ...
    return (
      <div className="grid grid-cols-11 gap-5">
        <div className="col-span-5 bg-white rounded-lg p-5 border border-[#EDEEF0]">
          <div className="p-3 rounded-lg mb-4" style={{ backgroundColor: 'var(--info-50)', borderLeft: '4px solid #0891B2', fontSize: 13, color: '#0E7490' }}>Email Request Received · From: Suresh Nair · 10 Jun 2026 · 09:30 AM · Loan ID: LO000050 · Applicant: Prakash Shinde</div>
          ...
          ].map(([label, value]) => <div key={label} className="mb-3"><label style={{ fontSize: 12, color: 'var(--neutral-700)', fontWeight: 700 }}>{label}</label><input readOnly value={value} className="w-full mt-1 px-3 rounded-lg border border-[#D1D5DB] bg-[#F7F8FA]" style={{ height: 38, fontSize: 13 }} /></div>)}
  ...
  const renderRepayments = () => {
    if (activePage === 'treasury-deductions') {
      return (
        <div className="space-y-5">
          <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--neutral-100)', borderLeft: '4px solid #1A3C2A', fontSize: 13, color: 'var(--neutral-700)' }}><strong>Tri-Party Repayment Flow:</strong> Farmer sells produce → Subsidiary deducts repayment → transfers to SFPCL → SFPCL posts to SAP. Amounts must match within ₹1 tolerance.</div>
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--info-50)', borderLeft: '4px solid #0891B2', fontSize: 13, color: '#0E7490' }}>Repayment allocation is hardcoded: partial repayments apply to principal first, then interest. Full repayment triggers NOC workflow to Compliance.</div>
  ...
  const renderReconciliation = () => (
    <div className="space-y-5">
      ...
      <DataCard title="Matched and Unmatched Transactions"><SimpleTable headers={['Statement Date', 'Description', 'Statement Amount', 'SAP Entry', 'SAP Amount', 'Match', 'Action']}>{reconciliationRows.map(row => <tr key={row.desc} onClick={() => onNavigate('treasury-reconciliation')} className="border-b border-[#EDEEF0] clickable-row" style={{ backgroundColor: row.match ? 'var(--success-50)' : '#FEF2F2' }}><Cell>{row.date}</Cell><Cell>{row.desc}</Cell><Cell mono right>{formatCurrency(row.statement)}</Cell><Cell mono>{row.sap}</Cell><Cell mono right>{row.sapAmount ? formatCurrency(row.sapAmount) : '-'}</Cell><Cell>{row.match ? 'Matched' : 'Unmatched'}</Cell><Cell>{row.match ? 'Matched' : <button onClick={(e) => e.stopPropagation()} className="px-3 py-1.5 rounded-lg" style={{ backgroundColor: 'var(--error-500)', color: 'white', fontSize: 12, fontWeight: 700 }}>Match Manually</button>}</Cell></tr>)}</SimpleTable></DataCard>
    </div>
  );
...
function Header({ icon, title, action }: { icon: ReactNode; title: string; action?: ReactNode }) {
  return <div className="px-5 py-3 border-b border-[#EDEEF0] flex items-center justify-between" style={{ backgroundColor: 'var(--neutral-100)' }}><div className="flex items-center gap-2" style={{ color: 'var(--accent-treasury)' }}>{icon}<h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--neutral-900)' }}>{title}</h3></div>{action}</div>;
}
...
function Cell({ children, mono, blue, right }: { children: ReactNode; mono?: boolean; blue?: boolean; right?: boolean }) {
  return <td className={`px-4 py-3 ${right ? 'text-right' : ''}`} style={{ fontSize: 13, color: blue ? '#0E7490' : 'var(--neutral-700)', fontFamily: mono ? 'Roboto Mono' : 'inherit', fontWeight: blue ? 700 : 500 }}>{children}</td>;
}

function Info({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between py-2 border-b border-[#EDEEF0]"><span style={{ fontSize: 12, color: 'var(--neutral-500)' }}>{label}</span><span style={{ fontSize: 12, color: 'var(--neutral-900)', fontWeight: 700, textAlign: 'right' }}>{value}</span></div>;
}
