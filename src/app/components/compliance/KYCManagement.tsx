...
export function KYCManagement({ onNavigate, activePage }: KYCManagementProps) {
  ...
  return (
    <Shell
      ...
      actions={<button className="px-4 py-2.5 rounded-lg font-semibold flex items-center gap-2" style={{ backgroundColor: 'var(--brand-primary)', color: 'white', fontSize: '14px' }}><Download size={14} /> Export CSV</button>}
    >
      ...
      <div className="bg-white rounded-lg border border-[#EDEEF0] overflow-hidden table-scroll">
        <table className="w-full">
          <thead><tr style={{ backgroundColor: 'var(--neutral-100)', borderBottom: '1px solid #EDEEF0' }}>
            ...
            {['Member', 'Name', 'KYC Date', 'Expiry', 'Days Left', 'Status', 'Action'].map(h => <th key={h} className="px-4 py-3 text-left" style={{ fontSize: '11px', color: 'var(--neutral-400)', fontWeight: 800, textTransform: 'uppercase' }}>{h}</th>)}
          </tr></thead>
          <tbody>{rows.map(row => (
            <tr key={row.folio} onClick={() => setRequestMember(row)} className="border-b border-[#EDEEF0] clickable-row">
              ...
              <td className="px-4" style={{ fontSize: '13px', fontFamily: 'Roboto Mono', color: 'var(--brand-accent)', fontWeight: 800 }}>{row.folio}</td>
      ...
      {requestMember && (
        <AppModal
          ...
          footer={<><button onClick={() => setRequestMember(null)} className="px-4 py-2.5 rounded-lg border border-[#EDEEF0]">Cancel</button><button onClick={() => setRequestMember(null)} className="px-4 py-2.5 rounded-lg font-semibold" style={{ backgroundColor: 'var(--brand-primary)', color: 'white' }}>Send Request</button></>}
        >
