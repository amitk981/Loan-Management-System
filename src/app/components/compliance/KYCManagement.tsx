...
export function KYCManagement({ onNavigate, activePage }: KYCManagementProps) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-lg p-4 border border-[var(--neutral-200)] mb-4">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <div className="relative ml-auto">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Name / Folio / Village" className="pl-9 pr-3 rounded-lg border border-[var(--neutral-300)]" style={{ height: '36px', fontSize: '13px', minWidth: '260px' }} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        {[
          ...
          ['30-60 days', '17', 'var(--warning-800)', '30-60 days'],
          ['Current (>60 days)', '739', 'var(--success-500)', 'Current'],
        ].map(([label, value, color, nextStatus]) => (
          <button key={label} onClick={() => setStatus(nextStatus as typeof status)} className="bg-white rounded-lg p-4 border border-[var(--neutral-200)] text-left clickable-card">
      ...
      <div className="bg-white rounded-lg border border-[var(--neutral-200)] overflow-hidden table-scroll">
        <table className="w-full">
          <thead><tr style={{ backgroundColor: 'var(--neutral-100)', borderBottom: '1px solid var(--neutral-200)' }}>
          ...
          <tbody>{rows.length === 0 && (
            ...
            <tr key={row.folio} onClick={() => setRequestMember(row)} className="border-b border-[var(--neutral-200)] clickable-row">
              ...
              <td className="px-4" style={{ fontSize: '13px', color: row.daysLeft < 0 ? 'var(--error-500)' : row.daysLeft < 30 ? 'var(--warning-500)' : 'var(--warning-800)', fontWeight: 700 }}>{row.daysLeft < 0 ? `${row.daysLeft} days` : `${row.daysLeft} days`}</td>
